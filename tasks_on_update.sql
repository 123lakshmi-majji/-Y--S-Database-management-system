...
SELECT name,department,gender FROM employee
WHERE department='HR'

SELECT id,name,date_of_joining,department,gender FROM employee
WHERE gender='Female' AND department='Operations';

SELECT name,salary,department,gender FROM employee
WHERE gender='Male' AND (department='SW' OR department='Testing') AND salary>50000;

SELECT id,name,email,date_of_joining FROM employee
WHERE date_of_joining IS NULL;

SELECT id,department,date_of_joining FROM employee
WHERE department='SW' AND date_of_joining > TO_DATE('01-01-2020','DD-MM-YYYY');

SELECT  id,name,email,department FROM employee
WHERE email IS NULL AND department IS NULL;

SELECT id,name,gender,department,salary,date_of_joining FROM employee
WHERE gender='Male' AND department='Operations' AND Salary<=80000 AND date_of_joining < TO_DATE('15-07-2021','DD-MM-YYYY');

SELECT name,department,email,gender,salary FROM employee
...
SET salary=salary*
CASE
    WHERE department='HR' THEN 1.10
    WHERE department='SWE' THEN 1.20
    WHERE department='Operations' THEN 1.15
    WHERE department='Testing' THEN 1.08
    ELSE 1.00
END;
...
UPDATE employee
SET email='hr_dummy@example.com'
WHERE department='HR' AND email IS NULL;

SELECT * FROM employee 

-----------------------------------------------
--HR ,Operations,SW,Testing
--Used to provide distinct values
--Under a column
SELECT DISTINCT department from employee;
--Distinct ages
SELECT DISTINCT age FROM employee
--Distinct can be applied on multiple columns too
--When applied on multiple columns it picks the distinct combinations of those columns
SELECT DISTINCT department,age FROM employee;
------------------------------------------------------------------------------------
--FETCH FIRST
--Restrict the number of rows shown in result
...
SELECT id,name,salary
FROM employee
WHERE department in ('HR','SW');

SELECT id,name,salary
FROM employee
WHERE salary NOT IN(55000,50000,60000);
-------------------------------------------------------------------
--ORDER BY
--Used to sort the data in either in ASCENDING / DESCENDING ORDER based on column or multiple columns
--SELECT id,name,age FROM employee OPDER BY age
--The above query displays the records by performing a sorting on ascending order of age
--SELECT col1,col2,...FROM table_name OPDER BY
--Sorting by default takes place in ASCENDING order
--If you want to sort in descending order use DESC keyword explicitly
--In ASCENDING order NULLS are sorted last
...
--Use a multiple to break ties
--come from the use of previous columns

SELECT id,name,age
FROM employee
ORDER BY age ASC ,name DESC;

--ORDER BY can be done ona acolumn that isn't be
...
FROM employee
WHERE department='HR'
ORDER BY age,name;   -->Using name[ASC]as tie-breaker
---------------------------------------------------------------
...
--COUNT(columnname)->count of all the rows in that particular column that are having values
--It ignores null(It woon't count null)
--COUNT()works on any datatype
...
--SUM()
--SUM() can be only be applied on numeric types
SELECT SUM(age)
FROM employee
WHERE department='Testing';

--AVG()
--Finds to the average of a column with numeric values
SELECT AVG(salary)
FROM employee;
