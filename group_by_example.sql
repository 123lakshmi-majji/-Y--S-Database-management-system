...
-- Once the groups are formed, you can perform Aggregations on groups
-- Rules to follow when writing group by
-- 1. Any column that is used in SELECT clause, must either
--    a part of group by or aggregated (Used inside aggregation funs)
-- GROUP BY EXAMPLES
-- Group all the departments and fiding how many employees are
-- working in each department
SELECT department, COUNT(department)
FROM employee
GROUP BY department;

-- Correct way to count empployees working
-- under each department, if department can
-- have null values too
SELECT department, COUNT(*)
FROM employee
GROUP BY department;

-- Counting how many Male, Female employees
-- are there in the organization
SELECT gender, COUNT(*)
FROM employee
GROUP BY gender;
-- Count the employees working under each age group

SELECT age, COUNT(*)
FROM employee
GROUP BY age;

-- Find out the average age of employees
...
-- and arrage the result in the descending order of 
-- maximum salary
...
-- Find out the average age of each gender
-- from employees, arrange the result in ascending order
-- of average age
...
-- GROUP BY + Where
-- Grouping employees who joined after 01-09-2020
SELECT department, COUNT(*)
FROM employee
...
-- HAVING clause
-- HAVING clause is used to filter groups formed by GROUP BY
-- You can't use alias given in the SELECT statment in HAVING
-- Order or Execution
-- 1. From employee GROUP BY --> Form GROUPs based on department
-- 2. Calcuating aggregations
-- 3. Having takes place
-- 4. Select statment gets executed with aliases (if any)
SELECT department, MAX(salary)
FROM employee
GROUP BY department
...
FROM employee
GROUP BY department
HAVING mx_salary > 80000; -- Can't use alias in HAVING

-- Follwoing correct syntax when you have almost all basic clasues
-- SELECT, FROM, WHERE, GROUP BY, HAVING , ORDER BY
-- 1. SELECT, 2. FROM, 3. WHERE, 4. GROUP BY, 5. HAVING, 6. ORDER BY

-- All clauses in one query
-- Following SELECT, FROM, WHERE, GROUP BY, HAVING ORDER BY
SELECT department, COUNT(*) AS dept_wise_count
FROM employee
WHERE date_of_joining > TO_DATE('01-09-2020', 'DD-MM-YYYY')
GROUP BY department
HAVING COUNT(*) > 13
ORDER BY dept_wise_count DESC;

-- Can we write group by using multiple columns
-- AEC ACET ACOE --> College
-- AEC+CSE, AEC+IT
-- ACET + CSE + Male, ACET + IT+ Male
-- ACET + CSE + Female, ACET + IT+ Female
-- ACOE + CSE, ACOE + IT
-- Dept + Age --> HR + 27, HR + 30, SW + 30
-- The counts of employees working under
-- different department and age combinations
...
SELECT department, age, gender, COUNT(*)
FROM employee
