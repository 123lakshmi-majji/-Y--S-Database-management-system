...
--CTE's
--Window Functions
--Revise all the types of subqueries
--CTE->Common table expressions
--Used to maintain readability across large queries by allowing to store intermediate table results as a dummy table
--Find out all the employees who are earning more than 50000 as salary
--Syntax of CTE:
--WITH dummy_table_name AS
...
FROM employee
WHERE salary > 50000)
SELECT department,COUNT(*) FROM dummy_table
GROUP BY department;

--WINDOW FUNCTIONS(Analytical Functions):
--Window functions returns one result per one row
--Aggregate functions (AVG(),MAX(),MIN())
--RANK() or DENSE_RANK():
--This is used to provide ranking to each row of our data based either on COLUMN or multiple columns
--Ranking employee based on their salary
...
--Rank() skips the ranks in case of a tie
SELECT id,name,age,department,salary,
...
--RANK() using PARTITION BY keyword
--partition by helps us to separate the data based on a column before ranking each row
--Ranking will be given separately for each partition
SELECT id,name,age,gender,department,salary,
 RANK() OVER(PARTITION BY department,gender ORDER BY salary DESC)AS rank
FROM employee
WHERE salary IS NOT NULL AND department IS NOT NULL AND gender IS NOT NULL;
...
FROM employee)
SELECT * FROM dummy WHERE rank < 4;
...
SELECT id,name,age,department,gender,salary,
RANK() OVER(PARTITION BY department,gender ORDER BY salary DESC) AS Rank
FROM employee
WHERE salary IS NOT NULL;

--LEAD() && LAG():
--LEAD() is used to get the next value for a given row of a specific column
--LAG() is used to get the previous value for a given row of a specific column
...
SELECT id,name,department,salary,
LEAD(salary,1,0)OVER(ORDER BY salary DESC) AS prev_salary
FROM employee 
WHERE salary IS NOT NULL;
...
--SUM(),MIN(),MAX(),AVG() can also be used as window functions
