...
--Store a query as a view so that you don't have to write again and again
--We can query the view to get tha data that is frequently used
--Creating multile userd in data base
--Providing the users priviledges

--Creating view for all sw employees
CREATE VIEW sw_employees AS
SELECT * FROM employee WHERE department = 'SW';

SELECT *FROM sw_employees;

DELETE FROM employee WHERE id=1;

--Creating a view for showing the people in each department

CREATE VIEW dept_wise_count AS
SELECT department,COUNT(*) AS count_
FROM employee
GROUP BY department;

--Getting data from dept_wise_count view
SELECT * FROM dept_wise_count;

--Updatable views
--1.The view must be created based on a single table
--2.When a view doesn't contain any single row or aggregate funcions
--3.A view that is not containing UNION,INTERSECTION,MINUS,UNION ALL,DISTINCT
--You can use update command on the views
--Updating a view chnages the data in the underlying table on which ths view is created

SELECT * FROM sw_employees;
--Update the view called as sw_employees
UPDATE sw_employees SET email='example.com' WHERE id=10;
SELECT * FROM sw_employees WHERE id=10;
--The change takes place on source table too
SELECT * FROM employee WHERE department='sw';

--Views are used for restricting the access to specific columns from a table by excluding the access to those columns
--Example:

--Testing department has an operator
--Create a view with only specific columns
CREATE VIEW testing_dept_employees AS
SELECT id,name,age,gender,department,email
FROM employee WHERE department='Testing';

SELECT * FROM testing_dept_employees;

--NonUpdateable:
--A view that contains GROUP BY or single row of functions or aggregate functions
--A view with DISTINCT,set operator keywords

SELECT * FROM dept_wise_count;
--Fails
UPDATE dept_wise_count  SET count_=40 WHERE department='sw';

--Command to check all the views you created
SELECT * FROM user_views;
--Command to check currently which user is working
SHOW user;
--Command to check the tables created by current_user
